...
void draw_mesh_2(rabbit::opengl_drawer &drawer, rabbit::mesh<float> &mesh,
                 std::vector<std::pair<rabbit::vec3f, rabbit::vec3f>> vertices,
                 rabbit::mat4f model, int loc) {
  drawer.uniform_mat4f(loc, model);
  drawer.draw_triangles((float *)vertices.data(), vertices.size(),
...
static ovrLayerProjection2 renderer_render_frame(struct renderer *renderer,
  ...
  for (int i = 0; i < VRAPI_FRAME_LAYER_EYE_MAX; ++i) {
    ...
    __android_log_print(ANDROID_LOG_VERBOSE, "hello_quest", "GL_INVALID_VALUE");
    break; case GL_INVALID_OPERATION: __android_log_print(ANDROID_LOG_VERBOSE,
    "hello_quest", "GL_INVALID_OPERATION"); break; case
    GL_INVALID_FRAMEBUFFER_OPERATION: __android_log_print(ANDROID_LOG_VERBOSE,
    "hello_quest", "GL_INVALID_FRAMEBUFFER_OPERATION"); break; case
    GL_OUT_OF_MEMORY: __android_log_print(ANDROID_LOG_VERBOSE, "hello_quest",
    ...
    drawer.uniform_mat4f(model_matrix_loc, ralgo::mov3<float>(ppp).to_mat4());
    ...
    for (auto &p : PoseMap_) {
      ...
      ralgo::pose3<float> grip{{_q.x, _q.y, _q.z, _q.w}, {_p.x, _p.y, _p.z}};

      info("{%f,%f,%f,%f}, {%f,%f,%f}", _q.x, _q.y, _q.z, _q.w, _p.x, _p.y,
           _p.z);
...
static void app_handle_input(struct app *app) {
  ...
  while (vrapi_EnumerateInputDevices(app->ovr, i, &capability) >= 0) {
    ...
    if (capability.Type == ovrControllerType_TrackedRemote) {
      ...
      vrapi_GetInputTrackingState(app->ovr, capability.DeviceID, 0, &tracking);
